...
---
styler --> formats your R code according to style guide.
...
```
notice x and y variables hwere are dependent. if we ask what is mpg of a car of 4000lbs, we would estimate it to be around 15-16. Can we describe the dependence?


Example: What can we say about rear axle ratio if we know something about quarter mile time? Not much. not gaining much information , no trend visible. no dependence here.
...
```
^ usually needs to be tidy data, where each row represents 1 unit.
...
predict something about y using variables x1 and x2 to xp. 

__Data__: The data frame. (tidy data usually)
...
unclass(my_lm)
names(my_lm) #to see what the names of the my_lm are
my_lm$coefficients
...
```{r}
summary(my_lm) %>% #more information about the fitted model.
  names()
```
...
```{r}
#predict(my_lm) %>% 
 # head() #uses first few loggdppercap of the gapminder dataset.

#gapminder %>% #uses the log(gdpPercap), extracting this as the x value and inputs this into the predict --> it returns the predicted y-value.
 # head( )

#predict(my_lm, newdata = tibble(gdpPercap = c(500,600,700))) --> you see the resulting y values that it predicts.
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    geom_point() +
    geom_smooth(method="lm") + #fits the model. wont give you coeffs or numeric results.
    scale_x_log10()
```
...
3. `glance`: extract statistical summaries about the model as a whole (1-row tibble of summary stats telling you the fit of the model).
    - Useful for checking goodness of fit.

Exercise: apply all three functions to our fitted model, `my_lm`. What do you see?

```{r}
tidy(my_lm) #returns df with slope, intercept of line. and error of these estimates & p-value corresponding
augment(my_lm) #binds new columns onto our original DF. can look at fitted vs. actual. and look at the residuals for these.
augment(my_lm, newdata = tibble(gdpPercap = c(400,500,600))) #note no life exp column here, we still see fitted and std erorr of the fit.
glance(my_lm) #how well does model fit the data.
```
