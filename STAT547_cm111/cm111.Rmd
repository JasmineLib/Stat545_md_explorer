...
```{r}
#gave just the raw text in between the tags.
#returns a vector of length 1 because we didn't parse it yet using nodes. (sectoin 2)
#function cat() removes the \n and renders text more nicely
simple_page %>%  
  html_text() %>% 
  cat()
```
...
```{r}
#for html_nodes ("p") returns a list of nodes containing all the p tags.
#html_text returns text between the p tags, and even removes the a = href tag. (a nested tag)
...
#nested a tag doesn't matter. will still work
simple_page %>% 
	html_nodes("a") %>% 
  html_text()

#more complex - select all tags with the wiki id.
simple_page %>% 
	html_nodes("#wiki") %>% 
  html_text()
```
...
```{r}
#the nested a tag appears on its own as well as in the overarching ptag.
simple_page %>% 
	html_nodes("*") %>% 
	html_text()
```
...
```{r}
#returns a vector of all the components found within the a node.
simple_page %>% 
	html_nodes("a") %>% 
  html_attrs()


#if you just want the href tag, you can specify a specific attribute:
simple_page %>% 
  html_nodes("a") %>% 
  html_attr(name = "href")
```
