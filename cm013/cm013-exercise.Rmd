...
p= ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave("my_plot.png", p) #press tab to see where it saves. you can also put the /   / /// file path. 
#for plot object: we are saving plot p into that file.
#jpg, bmp, pdf, png, diff file types. 
```
...
- Vector vs. raster: Images are stored on your computer as either _vector_ or _raster_.
    ...
    - __Vector__: represented as shapes and lines (infinitely scaleable). `pdf`, [`svg`](https://www.w3schools.com/graphics/svg_intro.asp).
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10) #you can err on safe side because ggplot will not plot values that are out of range. #why did this change scale on pop, and not gdppercap?? gdp percap on x axis. because the change was withing scale_colour, if we want to chagne gdppercap, then make change within scale_x_log10()
    ) +
...
```{r}
#RColorBrewer has nice colour schemes. qualitative colour palette - for discrete variables (ie want more diff colours), sequential (small to large numbers), diverging (positive vs. neg ie red vs blue = hot/cold)
## All palettes the come with RColorBrewer:
#install.packages("RColorBrewer")
#at top we see sequential, middle is differential,and bottom is diverging.
#can use scale_colour_distiller or scale_colour_brewer - use these to choose palette.
...
p_scales +
    ...
    scale_colour_viridis_c(
        ...
        breaks  = 10^(1:10),
        labels  = comma_format()
        #use option = " " to choose your palette in viridis
    ) +
    scale_y_continuous(breaks=10*(1:10))
```
...
ggplot(iris, aes(Sepal.Width, Sepal.Length)) +
     ...
     labs(x = "Sepal Width",
          ...
          title = "Sepal sizes of three plant species") +
    theme_bw() +
    theme(axis.text = element_text(size = 16), #can specify font size, colour, italics, justification in element_text
          strip.background = element_rect(fill= "orange") +
            panel.background = element_rect(fill = "blue") ) #can specify features of rectangle like colour, opacity, fill, )
```
